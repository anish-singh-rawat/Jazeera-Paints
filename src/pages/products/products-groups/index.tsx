...
import { defaultValues } from "src/types/forms/productGroupTypes";
...
import {
  ...
  productGroupUpdate,
  productGroupToggle,
} from "src/store/apps/products/group/products_group";
...
import ProductsGroupDataTable from "src/components/products/group/ProductsGroupDataTable";
...
const Groups = () => {
  ...
  useEffect(() => {
    ...
    dispatch(sequenceMappingCodeSearch({ entityType: "PRODUCT_GROUP_CODE" }));
  }, []);

  useEffect(() => {
    setProductGroups(productGroup?.data);
  }, [productGroup]);
  ...
  const schema = yup.object().shape({
    code: sequenceMapData?.autoGeneration
      ? yup.string()
      : RevestCodeFormValidator(productGroup.data, groupItem),
    name: yup
      .string()
      .required()
      .test("unique-name", "Name already exists", function (value) {
        return isNameUnique(productGroup.data, value);
      }),
    altName: yup
      .string()
      .required()
      .test("unique-altName", "Alt Name already exists", function (value) {
        return isAltNameUnique(productGroup.data, value);
      }),
  });
  ...
  const onSubmit = async (data: any, event: any) => {
    ...
    if (!data.id) {
      res = await dispatch(productGroupCreate(data));
    } else {
      res = await dispatch(productGroupUpdate(data));
    }
    ...
    <Card>
      ...
      <CommonDrawer open={open} toggle={handleCloseDrawer}>
        <div className={classes.drawerWrapper}>
          <CommonDrawerHeader
            title={
              groupItem.id ? t("EDIT_PRODUCT_GROUP") : t("NEW_PRODUCT_GROUP")
            }
          ...
          >
            ...
            <CommonFormActionButtons
              ...
              disabled={productGroup?.isLoading}
            />
          </form>
        </div>
      </CommonDrawer>
