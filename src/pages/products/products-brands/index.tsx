...
import { defaultValues } from "src/types/forms/productBrandTypes";
...
import ProductsBrandsDataTable from "src/components/products/brands/ProductsBrandsDataTable";
import {
  productBrandsCreate,
  productBrandsSearch,
  productBrandsToggle,
  productBrandsUpdate,
} from "src/store/apps/products/brand/products_brand";
...
const Brands = () => {
  ...
  const productBrand: any = useSelector(
    (state: RootState) => state.productBrands
  );
  ...
  useEffect(() => {
    ...
    dispatch(sequenceMappingCodeSearch({ entityType: "PRODUCT_BRAND_CODE" }));
  }, []);

  useEffect(() => {
    setProductBrands(productBrand?.data);
  }, [productBrand]);
  ...
  const schema = yup.object().shape({
    name: yup
      ...
      .test("unique-name", "Name already exists", function (value) {
        return isNameUnique(productBrand.data, value);
      }),
    altName: yup
      .string()
      .required()
      .test("unique-altName", "Alt Name already exists", function (value) {
        return isAltNameUnique(productBrand.data, value);
      }),
      code: sequenceMapData?.autoGeneration
      ? yup.string()
      : RevestCodeFormValidator(productBrand?.data, groupItem),
  });
  ...
  const onSubmit = async (data: any, event: any) => {
    ...
    let res: any;
    if (!data.id) {
      res = await dispatch(productBrandsCreate(data));
    } else {
      res = await dispatch(productBrandsUpdate(data));
    }
    ...
    <Card>
      ...
      <CommonDrawer open={open} toggle={handleCloseDrawer}>
        <div className={classes.drawerWrapper}>
          <CommonDrawerHeader
            title={
              groupItem.id ? t("EDIT_PRODUCT_BRAND") : t("NEW_PRODUCT_BRAND")
            }
          ...
          >
            ...
            <CommonFormActionButtons
              ...
              disabled={productBrand.isLoading}
            />
          </form>
        </div>
      </CommonDrawer>
