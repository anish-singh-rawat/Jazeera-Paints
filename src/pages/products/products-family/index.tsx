...
import { defaultValues } from "src/types/forms/productFamilyTypes";
...
import ProductsFamilyDataTable from "src/components/products/family/ProductsFamilyDataTable";
import {
  productFamilyCreate,
  productFamilySearch,
  productFamilyToggle,
  productFamilyUpdate,
} from "src/store/apps/products/family/products_family";
...
const Family = () => {
  ...
  const [productFamilys, setFamilys] = useState<any>([]);

  const classes = useStyles();
  const { t } = useTranslation();
  const dispatch = useDispatch<AppDispatch>();

  const productFamily: any = useSelector(
  ...
  useEffect(() => {
    ...
    dispatch(sequenceMappingCodeSearch({ entityType: "PRODUCT_FAMILY_CODE" }));
  }, []);

  useEffect(() => {
    setFamilys(productFamily?.data);
  }, [productFamily]);
  ...
  const schema = yup.object().shape({
    name: yup
      ...
      .test("unique-name", "Name already exists", function (value) {
        return isNameUnique(productFamily.data, value);
      }),
    altName: yup
      .string()
      .required()
      .test("unique-altName", "Alt Name already exists", function (value) {
        return isAltNameUnique(productFamily.data, value);
      }),
      code: sequenceMapData?.autoGeneration
      ? yup.string()
      : RevestCodeFormValidator(productFamily.data, groupItem),
  });
  ...
  const onSubmit = async (data: any, event: any) => {
    ...
    if (!data.id) {
      res = await dispatch(productFamilyCreate(data));
    } else {
      res = await dispatch(productFamilyUpdate(data));
    }
    ...
    <Card>
      ...
      <CommonDrawer open={open} toggle={handleCloseDrawer}>
        <div className={classes.drawerWrapper}>
          <CommonDrawerHeader
            title={
              groupItem.id ? t("EDIT_PRODUCT_FAMILY") : t("NEW_PRODUCT_FAMILY")
            }
          ...
          >
            ...
            <CommonFormActionButtons
              ...
              disabled={productFamily.isLoading}
            />
          </form>
        </div>
      </CommonDrawer>
