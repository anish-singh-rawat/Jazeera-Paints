...
import CitiesDataTable from "src/components/generalSetup/cities/CitiesDataTable";
...
const index = () => {
  ...
  const schema = yup.object().shape({
    ...
    name: RevestNameValidator(globalCity?.data, item || "altName"),
    altName: RevestAltNameFormValidator(globalCity?.data, item),
  ...
  const onSubmit = async (data: any, event: any) => {
    ...
    if (!data.id) {
      res = await dispatch(cityCreate(requestData));
    } else {
      res = await dispatch(cityUpdate(requestData));
    }
    handleCloseDrawer();
  };
  ...
  return (
    <Card>
      <CitiesDataTable
        data={globalCity.data ?? []}
        isLoading={globalCity?.isLoading}
      ...
      <CommonDrawer open={open} toggle={handleCloseDrawer}>
        <div className={classes.drawerWrapper}>
          <CommonDrawerHeader
            title={item.id ? t("EDIT_CITY") : t("NEW_CITY")}
