...
interface CommonAuthenticationCodeProps {
  // Add a background prop for the image URL
  heading: string;
  title: string;
...
const CommonAuthenticationCode: React.FC<CommonAuthenticationCodeProps> = ({
  heading,
  title,
  children,
  height,
  ...
  const BoxRapper = styled("div")(({ theme }) => ({
    backgroundImage: `url(/images/pages/background_image.png)`, // Set the background image
    backgroundSize: `600px ${height}`, // Adjust background size as needed
  ...
  const CardRapper = styled("div")(({ theme }) => ({
    width: "450px",
  ...
  const OuterRapper = styled("div")(({ theme }) => ({
    display: "flex",
    justifyContent: "center",
    alignItems: "center",
    height: "600px",
  ...
  return (
    <div>
      <Box sx={{ position: "absolute", right: "80px", top: "40px" }}>
...
export default CommonAuthenticationCode;
