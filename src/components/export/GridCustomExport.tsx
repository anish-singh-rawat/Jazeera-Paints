...
const GridCustomExport = (props: any) => {
  const { t } = useTranslation();
  ...
  if (unitOfMeasure) searchText = t("SEARCH");
  if (pointOfSale) searchText = t("SEARCH_WITH_NAME_DEVICE");
  if (country) searchText = t("COUNTRY_SEARCH");
  ...
  useEffect(() => {
    if (search) {
      ...
      if (isTranslationPage) {
        let filterData: any = rows?.filter((ele: any) => {
        ...
        let filterData: any = rows?.filter((ele: any) => {
          // come later here to change keys
          if (
            ele["profileName"]?.toLowerCase()?.includes(search?.toLowerCase())
          ) {
            return ele;
          }
        });
        setData(filterData);
      } else if (customFieldsList) {
        let filterData: any = rows?.filter((ele: any) => {
          // come later here to change keys
          if (
            ele["name"]?.toLowerCase()?.includes(search?.toLowerCase()) ||
        ...
        let filterData: any = rows?.filter((ele: any) => {
          ...
          if (
            ...
            ele["role"]?.name?.toLowerCase()?.includes(search?.toLowerCase())
          ) {
            return ele;
          }
        });
        ...
        let filterData: any = rows?.filter((ele: any) => {
          // come later here to change keys
          if (
            ele["code"]?.toLowerCase()?.includes(search?.toLowerCase()) ||
            ele["name"]?.toLowerCase()?.includes(search?.toLowerCase()) ||
            ele["unitClass"]?.toLowerCase()?.includes(search?.toLowerCase())
          ) {
            return ele;
          }
        });
        ...
        let filterData: any = rows?.filter((ele: any) => {
          if (
            ...
            ele["altName"]?.toLowerCase()?.includes(search?.toLowerCase()) ||
            ele["code"]?.toLowerCase()?.includes(search?.toLowerCase()) ||
            ele["productCategory"]?.["name"]
              ?.toLowerCase()
              ?.includes(search?.toLowerCase()) ||
            ele["products"]?.["shortName"]
              ?.toLowerCase()
              ?.includes(search?.toLowerCase()) ||
            ele["serialNumber"]?.toLowerCase()?.includes(search?.toLowerCase())
          ) {
            console.log("running");
            return ele;
          }
  ...
  const handleSearch = (value: string) => {
    setSearch(value);
    if (typeof props?.searchStateHandler == "function")
      props.searchStateHandler(value);
  };
                ...
                <>
                  {isHeaderRequired && (
                    <GridCustomHeader
                      ...
                      searchText={searchPlaceholder ?? searchText}
                      // name={name}
                      showHeaderActionBtn={showHeaderActionBtn}
...
export default GridCustomExport;
