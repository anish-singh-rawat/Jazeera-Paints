...
//convert date into hh:mm AM/PM
export function hours12(date: any) {
...
export const customNumber = (e: any) => {
  const re = /^[0-9]*$/;
  if (e.target.value === "" || re.test(e.target.value)) {
    return e.target.value;
  } else {
...
export const regularExpressionValidator = (e: any, regX: any) => {
  const obj = new RegExp(regX);
  if (obj.test(e.target.value)) {
    return e.target.value;
  }
};

export const getDateFormate = (date: any) => {
